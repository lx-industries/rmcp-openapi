...
fn create_server_with_custom_spec(base_url: Url) -> anyhow::Result<Server> {
    // Create custom OpenAPI spec with optional array parameters for testing
    let json_value = json!({
        "openapi": "3.0.0",
        "info": {
            "title": "Test Petstore",
        ...
        "paths": {
            "/pet/search": {
                "get": {
                    ...
                    "summary": "Search pets with optional arrays",
                    "parameters": [
                        {
                            "name": "name",
                            "in": "query",
...
/// Test empty optional array parameters are omitted from HTTP requests
#[tokio::test]
async fn test_empty_optional_array_parameters_omitted() -> anyhow::Result<()> {
    let mut mock_server = MockPetstoreServer::new_with_port(9109).await;

    // Mock an endpoint that expects NO empty optional arrays in the request
    let mock = mock_server.mock_search_pets_without_optional_arrays();

    let server = create_server_with_custom_spec(mock_server.base_url())?;
    let client = HttpClient::new().with_base_url(mock_server.base_url())?;

    let tool_metadata = server
        .get_tool_metadata("searchPets")
        .expect("searchPets tool should be registered");

    // Send request with empty optional arrays
    let arguments = json!({
        "name": "test",
        "tags": [],  // Optional array - should be omitted
        "categories": []  // Optional array - should be omitted
    });

    let response = client.execute_tool_call(tool_metadata, &arguments).await?;

    // Verify the request succeeded (mock expects no empty arrays)
    assert!(response.is_success);
    assert_eq!(response.status_code, 200);

    // Verify empty arrays were omitted from the URL
    assert!(response.request_url.contains("name=test"));
    assert!(!response.request_url.contains("tags="));
    assert!(!response.request_url.contains("categories="));

    // Verify the mock was called (validates our expectations)
    mock.assert();

    Ok(())
}

/// Test non-empty optional array parameters are included in HTTP requests
#[tokio::test]
async fn test_non_empty_optional_array_parameters_included() -> anyhow::Result<()> {
    let mut mock_server = MockPetstoreServer::new_with_port(9110).await;

    // Mock an endpoint that expects non-empty optional arrays in the request
    let mock = mock_server.mock_search_pets_with_optional_arrays();
    ...
    // Send request with non-empty optional arrays
    let arguments = json!({
        "name": "test",
        "tags": ["friendly", "small"],  // Non-empty optional array - should be included
        "categories": ["dog"]  // Non-empty optional array - should be included
    });
    ...
    // Verify non-empty arrays were included in the URL
    assert!(response.request_url.contains("name=test"));
    assert!(response.request_url.contains("tags=friendly"));
    assert!(response.request_url.contains("tags=small"));
    assert!(response.request_url.contains("categories=dog"));
...
/// Test required array parameters are always included even when empty
#[tokio::test]
async fn test_required_array_parameters_always_included() -> anyhow::Result<()> {
    let mut mock_server = MockPetstoreServer::new_with_port(9111).await;

    // Mock an endpoint that expects required arrays even when empty
    let mock = mock_server.mock_find_pets_by_status_empty();
    ...
    // Send request with empty required array (should still be included)
    let arguments = json!({
        "status": []  // Required array - should be included even when empty
    });
    ...
    // Our implementation is working correctly - required empty arrays are included as "status="
    // This ensures the parameter is present in the request even when empty.
    assert!(response.request_url.contains("status="));
...
impl MockPetstoreServer {
    ...
    /// Mock searchPets endpoint that expects NO optional array parameters
    pub fn mock_search_pets_without_optional_arrays(&mut self) -> Mock {
        let pets_response = json!([
            {
                "id": 1,
                "name": "test",
        ...
        self.server
            .mock("GET", "/pet/search")
            .match_query(mockito::Matcher::Exact("name=test".to_string()))
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(pets_response.to_string())
            .create()
    ...
    /// Mock searchPets endpoint that expects non-empty optional arrays
    pub fn mock_search_pets_with_optional_arrays(&mut self) -> Mock {
        let pets_response = json!([
            {
                "id": 1,
                "name": "test",
        ...
        self.server
            .mock("GET", "/pet/search")
            .match_query(mockito::Matcher::Regex(r".*name=test.*".to_string()))
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(pets_response.to_string())
            .create()
    ...
    /// Mock findPetsByStatus endpoint that accepts empty required arrays
    pub fn mock_find_pets_by_status_empty(&mut self) -> Mock {
        let pets_response = json!([]);
